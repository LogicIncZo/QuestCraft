...
const GamePage: React.FC<GamePageProps> = ({ questConfig, onExit, onOpenFooterDrawer }) => {
    ...
    const updateGameState = (newState: Partial<Omit<GameState, 'players'>>) => {
        setGameState((prev) => {
            ...
            if (prev.gamePhase !== nextState.gamePhase) {
                logger.info(
                    `[Game] Game phase changed from ${prev.gamePhase} to ${nextState.gamePhase}`
                );
            }
            return nextState;
        });
    ...
    const initializePlayers = useCallback(
        (count: number, names: string[], mode: GameMode) => {
            const initialPlayers: Player[] = Array.from({ length: count }, (_, i) => ({
                ...
                position: 0,
                resources: questConfig.resources.reduce(
                    (acc, resource) => {
                        acc[getLocalizedString(resource.name, 'en').toLowerCase()] =
        ...
        (changes: ResourceChange[]) => {
            setPlayers((prevPlayers) => {
                ...
                for (const change of changes) {
                    ...
                    const resourceDef = questConfig.resources.find(
                        (r) => getLocalizedString(r.name, 'en').toLowerCase() === resourceName
                    );
            ...
            switch (location.type) {
                ...
                case BoardLocationType.CHANCE:
                    if (questConfig.chanceCards.length > 0) {
                        const card =
                            questConfig.chanceCards[
                                Math.floor(Math.random() * questConfig.chanceCards.length)
                            ];
                ...
                case BoardLocationType.COMMUNITY_CHEST:
                    if (
                        questConfig.communityChestCards &&
                        questConfig.communityChestCards.length > 0
                    ) {
                        const card =
                            questConfig.communityChestCards[
                                Math.floor(Math.random() * questConfig.communityChestCards.length)
                            ];
                    ...
                    setPlayers((ps) =>
                        ps.map((p, i) =>
                            i === currentPlayerIndex
                                ? {
                                      ...p,
                                      position: questConfig.board.jailPosition,
                                      inJail: true,
                                      jailTurns: 0,
                                  }
                                : p
    ...
    const handleRollDice = useCallback(() => {
        ...
        setTimeout(() => {
            ...
            const currentPlayer = players[currentPlayerIndex];
            const newPosition =
                (currentPlayer.position + totalRoll) % questConfig.board.locations.length;

            setPlayers((ps) =>
                ps.map((p, i) => (i === currentPlayerIndex ? { ...p, position: newPosition } : p))
            );
    ...
    const handleAIChoice = useCallback(
        async (scenario: ManagedScenario) => {
            try {
                const choiceIndex = await getAIChoice(
                    questConfig,
                    scenario,
                    players[currentPlayerIndex]
                );
    ...
    const commonGameBoard = (
        <GameBoard
            board={questConfig.board}
            players={players}
            questName={getLocalizedString(questConfig.name, language)}
            language={language}
        />
    );
    ...
    return (
        <div className="h-full">
            ...
            <div className="hidden xl:grid h-full grid-cols-1 xl:grid-cols-4 gap-4 p-4">
                <div className="xl:col-span-1 xl:order-1">{commonPlayerDashboard}</div>
                <div className="xl:col-span-2 xl:order-2 flex items-center justify-center">
                    {commonGameBoard}
                </div>
                <div className="xl:col-span-1 xl:order-3">{commonActionPanel}</div>
            </div>

            {/* Tablet Layout (md to xl) */}
            <div className="hidden md:grid xl:hidden h-full grid-cols-3 gap-4 p-4">
                <div className="col-span-2 flex items-center justify-center">{commonGameBoard}</div>
                <div className="col-span-1 flex flex-col gap-4 overflow-hidden">
                    <div className="flex-1 min-h-0">{commonPlayerDashboard}</div>

...
describe('storageService.safeSetItem quota path (issue #55)', () => {
    ...
    it('returns true on success and false on any storage failure', () => {
        let backing = '';
        const ok = { getItem: () => backing, setItem: (_k: string, v: string) => { backing = v; }, removeItem: () => { backing = ''; } } as unknown as Storage;
        expect(safeSetItem(ok, 'k', 'v')).toBe(true);
        const broken = { getItem: () => null, setItem: () => { throw new Error('SecurityError'); }, removeItem: () => {} } as unknown as Storage;
    ...
    it('setVersionedItem tags schemaVersion and getVersionedItem rejects mismatches', () => {
        ...
        const s = { getItem: (k: string) => store.get(k) ?? null, setItem: (k: string, v: string) => { store.set(k, v); }, removeItem: (k: string) => { store.delete(k); } } as unknown as Storage;
...
describe('settingsService versioning (issue #55)', () => {
    ...
    it('saves settingsVersion and reads it back', () => {
        settingsService.saveSettings({ ai: { providerId: 'community', model: 'm', baseUrl: 'b', aiRequestDelayMs: 0 }, language: 'en' } as any);
    ...
    it('migrates legacy settings without settingsVersion', () => {
        localStorage.setItem('questcraft-app-settings', JSON.stringify({ ai: { providerId: 'openrouter', apiKey: 'sk-legacy', model: 'x', baseUrl: 'y' }, language: 'en' }));

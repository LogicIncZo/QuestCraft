...
import { safeSetItem, safeGetItem, safeRemoveItem, setVersionedItem, getVersionedItem } from './storageService';
...
export const gameStateService = {
  save(state: GameState): void {
    ...
    if (!persisted) {
      logger.error('[GameState] Game state could not be persisted (storage unavailable or full). Progress is kept in memory for this session only.');
    }
  },
